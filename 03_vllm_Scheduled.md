...
* Prefill 很长，本质上只是“欠的 token 特别多”
* Decode 很轻，本质上只是“每轮只欠很少 token”
...
> **Scheduler 在一个时间步里，如何决定“谁先算、算多少”**

那么下一步最自然的问题就是：

* 这些 `SchedulerOutput` 具体是怎么交给 `Executor` 的？
* `Worker` 和 `Executor` 的职责边界到底在哪里？
* 为什么 `Engine` 不自己直接做模型执行，而要再套一层执行器抽象？
